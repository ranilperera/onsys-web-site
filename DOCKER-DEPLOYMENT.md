...
| Forms fail with a CORS error on one hostname | Visitor is on the non-canonical host (fixed — the API now accepts both), and the proxy is not redirecting | Add the canonical-host redirect in §7 |

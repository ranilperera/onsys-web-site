...
import { chatStartSchema, chatMessageSchema, chatEscalateSchema } from '@onsys/shared';
...
): Promise<{ reply: string; status: 'WAITING_HUMAN' | 'BOT' }> {
  const session = await prisma.chatSession.findUnique({
    where: { id: sessionId },
  ...
  if (result.delivered) {
    await prisma.chatSession.update({
      ...
      data: {
        ...
        teamsThreadId: result.threadId ?? undefined,
      },
    });
